...
- **NOT code-fixable here:** "All charts not loading" (needs the owner's browser console — see the reply; likely CDN/SRI since every chart path is `safeChart_`-guarded) and "Field Ops Power / Denials queues not showing" (data/mapping — QCD col D spellings vs the dept's effective queue list; Dept Config modal discovered-queues is the fix surface, Op State #14).
