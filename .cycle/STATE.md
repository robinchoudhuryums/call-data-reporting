...
- **PHASE C cycle 2 REMAINS (approved):** C3 Trends consolidation (single metric dropdown incl. queue metrics + [Line|Calendar] + [Monthly|Daily] hidden in Calendar; FIX the calendar day-click no-op for Answer%/Missed — the delegated listener on static #ins-trend-calendar LOOKS fine, reproduce live; ADD call-volume calendar metric from trendDaily); C4 agent-scoped missed chart (client rebucket from agent timelines + clear chip); C5 Direct dept cards (client rollups from per-agent rows — payload already carries dept per agent; expand-in-place tables; CSV gains a Department column, owner-confirmed).
