...
## Latest session (R11 Phase B — third testing pass, owner-approved plan; B1..B11 SHIPPED)
Branch `claude/broad-scan-ak8g04`, **434/434 tests** (queue-report email pins rewritten for R11-B4: banner-retired + split-bar share assertions + Denials 50% case), INV-16 green, script.html parses. Deploy: Department Dashboard ONLY.
...
- **PHASE C REMAINS (approved, not started):** C1 CSR tile delta chips (prior-window avgAnswer+transfer, summary:v15); C2 Insights dwell-prefetch (shared request-shape resolver + reverse dept prefetch); C3 Trends consolidation (single metric dropdown incl. queue metrics + [Line|Calendar] + [Monthly|Daily] hidden in Calendar; FIX the calendar day-click no-op for Answer%/Missed; ADD call-volume calendar metric); C4 agent-scoped missed chart (client rebucket from agent timelines + clear chip); C5 Direct dept cards (client-side rollups from per-agent rows, expand-in-place tables, CSV gains a Department column). Suggested order: C1+C2 one cycle, then C3+C4+C5.
- **Neon May 6–26 diagnosis PENDING owner checks:** date-scoped SQL count + parity run with DQE_PARITY_FROM/TO=2026-05-06/26 watching "sheet rows:" (a ~0 count proves the date-cell parse failure theory → repair cells, not re-process; Call_Legs retention blocks daily re-import for May — bulk CSV backfill + DQE_UPSERT_SINCE floor is the re-process route; the dup-guard re-mirror answer was given).
- Where I left off: committed/pushed; not deployed. Live smoke: QCD verdict band + banner tones (CSR green @0.82% w/ Spanish row red), Export ▾ menus (QCD modal), email preview (no banner, split bars, single-date subject, no Courier), Overview collapse chevron, missed bars filling + 8AM ticks + no toggle for managers, chart ? tips, Insights pinned header w/ period row, Inbound/Direct: no gap above stuck headers + column sorting.
