...
- **Field Ops Power / Denials Overview gap:** still open — owner confirms Dept Config + QCD col D spellings match (`A_Q_FieldOps_Power`, `A_Q_Denials`, non-zero rows). Next suspects to check in-session: `QCD_READ_SOURCE`/neon mirror coverage for those queues, the Overview snapshot window (`computeQcdSnapshots_` min(sinceIso, mtdStart) window — data older than window renders nothing), OVERVIEW_HIDDEN_DEPTS / parent mapping, dept-name-vs-roster-header mismatch (snapshot keys by dashboard dept name).
