...
item 1 | drive-smoke.js | Three assertions: Range is the default, MTD loads a different window and labels it, switching back restores the page-window figures. Clicks are dispatched IN-PAGE, not via `page.click` — the panel rides a sticky aside and can sit under the frost overlay, where Playwright's actionability wait times out (it did, and drive-smoke swallowed the throw silently until the run was checked for its exit code rather than its grep).
...
- `drive-smoke` swallows a driver exception per role and still prints a summary, so a thrown selector error reads as "checks passed" unless the exit code is checked. Worth making a throw fail loudly.
