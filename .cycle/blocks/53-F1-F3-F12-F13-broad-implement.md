...
- Run the F1 exposure probe against Neon: `SELECT count(*) FROM inbound_calls WHERE entry_queue IS NULL AND disposition='abandoned';` — a non-trivial count means F1 was ALREADY firing, and those dates need a re-import (within the ~14-day Call_Legs window) to heal. It is also a candidate contributor to the parked QCD-vs-inbound discrepancy that currently blocks the Inbound report's release to managers. | BLOCKS DEPLOY: N
