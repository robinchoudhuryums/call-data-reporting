...
// S2B-7 (broad-scan 2026-09-23): the team-avg excludes are ROSTER names, which
// are feed spellings and can lead with a formula character; the save wrote the
// joined list raw into a sheet cell.
test('S2B-7: sheetUpsertDeptConfigRow_ neutralizes a formula-leading team-avg-excludes cell', function () {
  setConfig([row({ dept: 'Alpha', qcd: 'A_Q_A' })]);
  // Util.gs is not loaded here (earlier tests stub sheetSafeCell_ as identity);
  // install its real rule for this assertion.
  h.ctx.sheetSafeCell_ = function (v) { return (typeof v === 'string' && /^[=+\-@\t\r]/.test(v)) ? "'" + v : v; };
  h.call('sheetUpsertDeptConfigRow_', {
    dept: 'Alpha', qcdQueues: ['A_Q_A'], overviewParent: '', teamAvgExcludes: ['=Evil Name', 'Bo'],
