...
/**
 * Manually checks all sheets and sends styled alerts for those below threshold.
 */
function sendManualAlert() {
  ...
  const sheetEmailMap = {
    ...
    "Resupply Q": "nikunj.salanki@universalmedsupply.com", "Field Ops Q": "mahesh.patel@universalmedsupply.com",
  ...
  Object.entries(sheetEmailMap).forEach(([sheetName, toEmail]) => {
    ...
    const b1DateStr = Utilities.formatDate(new Date(b1), timeZone, "MM/dd/yyyy");
    const threshold = sheetThresholdMap[sheetName] || 0.33;

    if (typeof c2 !== 'number' || isNaN(c2)) {
      skippedNonNumeric.push(`${sheetName} (C2 value: ${c2})`);
      return;
    }

    if (c2 >= threshold) {
      skipped.push(`${sheetName} (${(c2 * 100).toFixed(1)}%)`);
      return;
    }

    alerted.push(`${sheetName} (${(c2 * 100).toFixed(1)}%)`);
    ...
    const htmlBody = buildManualAlertEmailBody(sheetName, b1DateStr, c2, g2, filteredRows, ss.getUrl(), sheet.getSheetId());

    MailApp.sendEmail({
      to: toEmail,
      cc: MANUAL_ALERT_CONFIG.CC_EMAIL,
      subject: subject,
      htmlBody: htmlBody
    });

    let logSheet = ss.getSheetByName(MANUAL_ALERT_CONFIG.ALERT_LOG_SHEET);
    if (!logSheet) {
      logSheet = ss.insertSheet(MANUAL_ALERT_CONFIG.ALERT_LOG_SHEET);
      logSheet.hideSheet();
  ...
  let alertMsg = "Manual alert check complete.";
  if (alerted.length > 0) alertMsg += `\n\nAlerted:\n- ${alerted.join("\n- ")}`;
  if (skipped.length > 0) alertMsg += `\n\nSkipped (Above Threshold):\n- ${skipped.join("\n- ")}`;
  if (skippedNonNumeric.length > 0) alertMsg += `\n\nSkipped (Non-Numeric C2):\n- ${skippedNonNumeric.join("\n- ")}`;
  SpreadsheetApp.getUi().alert(alertMsg);
}

/**
 * NEW: Builds a styled HTML email body for the manual alert.
 */
function buildManualAlertEmailBody(sheetName, date, answerRate, abandonedCalls, agentData, spreadsheetUrl, sheetId) {
  let tableHtml = "";
  ...
  return `
    <div style="background-image: url('${MANUAL_ALERT_CONFIG.BACKGROUND_IMAGE_URL}'); background-color: #e9ecef; background-size: cover; padding: 40px; font-family: sans-serif;">
      <div style="background-color: rgba(255, 255, 255, 0.85); padding: 25px; border-radius: 8px;">
        <table style="width:100%; border-collapse:collapse; margin-bottom:15px;">
          <tr>
            <td style="width:60px; vertical-align:middle;"><img src="${MANUAL_ALERT_CONFIG.LOGO_URL}" alt="Company Logo" style="height:50px; display:block;"></td>
