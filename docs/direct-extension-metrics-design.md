...
Cached 30 min under `directCall:v3` (+ `kpisPrior`/`deptsPrior` for delta chips, R11-M; v3 groups the company-view agents per (agent, dept) so a crossover agent's rows stay in their own depts, B-1); unavailable payloads not cached. Client:
