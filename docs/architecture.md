...
|---|---|---|---|---|
| Main per-agent table | `Data.gs` | `getDepartmentSummary` | `summary:v22:` | no |
| Missed Calls Report | `MissedCallsReport.gs` | `getMissedCallsReport` | `missed:v18:` | no |
| Individual / Peer Comparison | `IndividualReport.gs` | `getIndividualReportInit`, `getIndividualReport`, `sendIndividualReportEmail` | `individual:v12:`, `individual_active:v2:` | no |
...
| Company Overview | `CompanyOverview.gs` | `getCompanyOverview` | `companyOverview:v24` | partial (admin-only `companyAggregate`, `pipelineFreshness`, `orphanNag`, `unmappedQcd` fields) |
| Overview YTD chart trend (the chart's own payload when the range control is YTD -- the 90-day series ride the Company Overview blob above; the full series would blow its ~100KB cap) | `CompanyOverview.gs` | `getOverviewChartTrend` | `overviewChartYtd:v3` | partial (admin-only `company` -- the Company aggregate LINE, weighted % answered + abandoned %; this endpoint is manager-or-admin with a SHARED cache, so unlike `companyAggregate` it cannot ride a wholesale strip: `ovStripChartTrend_` removes the field on serve, fail-closed, on all three return paths -- cache hit, degraded, fresh) |
| DQE-silence watchdog (push twin of the tile badge) | `DqeSilenceWatch.gs` | (trigger-driven; `runDqeSilenceCheckNow` admin RPC) | (no cache; state in `DQE_SILENCE_STREAKS`) | admin-only |
...
| Insights Report (period comparison: team rollup + per-agent cards) | `InsightsReport.gs` | `getInsightsReportInit`, `getInsightsReport`, `sendInsightsReportEmail` | `insights:v24:` | no (per-dept gate like IR/PR/CR) |
| Inbound Report (per-call inbound view from Neon `inbound_calls`) | `InboundReport.gs` | `getInboundReport`, `getInboundInsurerDaily`, `getInboundHeatmap` (weekday×hour abandon heatmap; degrades to the `Inbound Calls` SHEET when Neon is unreachable -- mirrored bucketing + two-arm dept attribution, never cached, disclosed via `meta.fallbackSource`, cell drill suppressed), `getCallJourney` (per-call path drill; manager fallback entitlement-gated via the dept's own Missed report, F-4; the inbound arm degrades to the `Inbound Calls` sheet cols 18-22 when Neon is unreachable -- same auth, disclosed, journey cells windowed by INBOUND_EXPORT_JOURNEY_DAYS; the outbound arm has no fallback. `kind:'outbound'` also serves the OUTBOUND call an internal assist was placed from -- the one place a dept sees another dept's customer call, gated by a SERVER-RE-DERIVED capability: an internal record must link to it AND itself be drillable by that manager. Owner ruling in docs/known-issues.md) | `inbound:v11:`, `inboundHeatmap:v4:` | TEMPORARILY admin-only while vetted (per-dept manager path kept intact); `getCallJourney` is manager-reachable for own dept |
| Direct Call Report (per-agent direct-extension metrics from Neon `direct_call_history`; DC-1: degrades to the `Direct Call History` SHEET when Neon is unreachable -- that sheet is the PRIMARY and Neon the mirror, so the fallback's figures are COMPLETE; shared shaper, never cached) | `DirectCallReport.gs` | `getDirectCallReport` | `directCall:v4:` | TEMPORARILY admin-only while the busy carve-out is vetted (per-dept manager path kept intact) |
| Outbound Report ("did we call back the ones who abandoned?" + per-agent outbound activity from Neon `outbound_calls`; abandon denominator = the Inbound report's Abandoned population verbatim) | `OutboundReport.gs` | `getOutboundReport`, `getOutboundUncalled` (not-called-back drill, uncached) | `outboundReport:v4:` | TEMPORARILY admin-only while vetted (per-dept manager path kept intact) |
...
| Orphan Fix engine (admin write path) | `OrphanFix.gs` | `getOrphanFixInit`, `addAgentAlias`, `removeAgentAlias`, `applyOrphanRename` | (no cache; busts `COMPANY_OVERVIEW_CACHE_KEY` -- currently `companyOverview:v24` -- on write) | yes |
