...
| Inbound Report (per-call inbound view from Neon `inbound_calls`) | `InboundReport.gs` | `getInboundReport`, `getInboundInsurerDaily`, `getInboundHeatmap` (weekday×hour abandon heatmap; degrades to the `Inbound Calls` SHEET when Neon is unreachable -- mirrored bucketing + two-arm dept attribution, never cached, disclosed via `meta.fallbackSource`, cell drill suppressed), `getCallJourney` (per-call path drill; manager fallback entitlement-gated via the dept's own Missed report, F-4. `kind:'outbound'` also serves the OUTBOUND call an internal assist was placed from -- the one place a dept sees another dept's customer call, gated by a SERVER-RE-DERIVED capability: an internal record must link to it AND itself be drillable by that manager. Owner ruling in docs/known-issues.md) | `inbound:v9:`, `inboundHeatmap:v3:` | TEMPORARILY admin-only while vetted (per-dept manager path kept intact); `getCallJourney` is manager-reachable for own dept |
