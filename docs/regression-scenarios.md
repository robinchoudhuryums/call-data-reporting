...
S32 | Queue data end-to-end (Insights Queue health + retained QCD surfaces) | Subsystem: Department Dashboard + CDR Import
  Steps:
    ...
    - Open the dashboard as a manager. Go to My Department and click "Insights" on the Agent table · Insights lens switcher (the top-nav Insights tab is retired, Round-16b) -- it AUTO-GENERATES the agent-free whole-dept report on first entry; use "Edit dates & agents" to move to a range with known QCD activity if needed.
...
S37 | Insights report end-to-end (comparison modes + CR-ported analytics) | Subsystem: Department Dashboard
  Steps:
    - Open the dashboard as a manager; go to My Department and click "Insights" on the Agent table · Insights lens switcher (the top-nav Insights tab is retired, Round-16b; per-dept gated server-side like IR/CR). The My Department tab keeps the header highlight while Insights is active.
    - First entry AUTO-GENERATES (default compare = "Immediately-preceding period"; no setup-form step -- the header From/To + Quick-select chips edit the window, the "Comparison & agents" popover edits the rest). Adjust to a range with activity + 2+ agents via the header dates + popover. Confirm: 6 KPI tiles with delta badges AND 12-month sparklines; the 12-Month Team Trend chart with its single Metric dropdown (R11-C3; admin sees the ATT + Queue entries); per-agent cards each carrying 6 metrics with their OWN delta badges; floaters get the QUEUE chip + warn border and are excluded from the rollup caption's roster-only totals (INV-53).
