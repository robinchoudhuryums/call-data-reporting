...
S14 | Insights team rollup: current vs prior deltas + PR-absorbed views | Subsystem: Department Dashboard
  Steps:
    - (The standalone Performance Report is RETIRED -- Insights is the replacement.) Open the Insights section (My Department, bottom) -- it AUTO-GENERATES the whole-department report on first open (no setup form). Set the DEPT controls' From/To to last month (M2: the dept row is the single date authority; the open region re-runs on the change) with no agents checked (whole department).
...
S18 | Insights length-mismatch surfaces per-day (ex-Compare Ranges, INV-35) | Subsystem: Department Dashboard
  Steps:
    - Open the Insights section (auto-generates on first open). Via "Comparison & agents", set Compare = Custom prior range with a prior window >= 1.2x longer (in working days) than the selected range; Apply.
    - Generate.
...
S32 | Queue data end-to-end (Insights Queue health + retained QCD surfaces) | Subsystem: Department Dashboard + CDR Import
  Steps:
    ...
    - Open the dashboard as a manager. Go to My Department and click "Insights" on the Agent table · Insights lens switcher -- since the M1 merge this opens + scrolls to the Insights SECTION at the bottom of the same page (expanding the collapsed "Insights" heading directly works too) and AUTO-GENERATES the agent-free whole-dept report on first open; move the DEPT From/To to a range with known QCD activity if needed (M2: the dept controls are the single date authority; the open region re-runs on the change).
