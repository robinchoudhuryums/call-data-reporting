...
- **Web app deploys as "Execute as: Me"** with **"Access: Anyone within
  ...
  inside every public function (known exception: `getLatestDataDate`/`getLatestDataDates` are currently ungated metadata reads -- CORE-1/DEEP-1, code fix pending; the F-28 commit message claimed a signed-in gate that never landed).
...
20. Neon keep-warm (optional; only relevant once `DQE_READ_SOURCE=neon`).
    ...
    are still on the sheet -- the warm ping cannot pollute it (F29). CAVEAT (NEO-3, code fix pending): the OTHER Neon surfaces (Inbound report/heatmap, Caller Lookup, escalation writes) DO record failures into the same `NEON_READ_LAST_ERROR`, and only the three DQE readers CLEAR it -- so while reads are on `sheet`, one transient non-DQE blip can pin a stale warn line indefinitely. Read the line as "some Neon call failed recently", not as strictly-DQE.
...
25. `CONFIG_SOURCE` Script Property (dashboard) -- the C2 Dept Config
    ...
    **C3 edit UIs SHIPPED:** the Alerts modal now has admin edit forms for both
    ...
    governs Dept + Alert + Digest config sources together.) The Alert + Digest compare gates read Neon DIRECTLY and return `clean:false` + an `error` when Neon is unreachable (F-5). CAVEAT (CORE-5, code fix pending): `compareDeptConfigSources` coerces an unreachable Neon to an EMPTY table -- no `error` field, and an empty sheet + Neon-down prints a false "PARITY CLEAN". Never flip `CONFIG_SOURCE` on a clean result from an empty sheet without confirming Neon was actually read; never flip on a result that carries `error`. Access Control is
