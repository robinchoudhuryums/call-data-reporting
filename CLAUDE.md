...
- **Insights period slider, trend-at-bottom, and the Insights<->My-Department
  ...
  `missed:v16` cache). It is the **DQE missed-ring lens** the heatmap cell
...
- **Role model + the all-departments manager (`allDepts`).** Three roles
  ...
  **all-departments manager** -- `role:'manager'` with a new
  ...
  gates but won't break the role the day those gates are removed.) **Grant it** by setting an Access
...
- **Scope is locked to `roster` (Phase D → redesign cleanup →
  ...
  (missed:v16) a sentinel is included when its queue NAME is in the
  dept's effective queue list (`queuesForDept_`, case-insensitive) --
  NOT via shared-extension overlap, which leaked other depts' queues
  onto the card. A dept with no mapped queues (or no queue-only
  abandons on its own queues in range) renders no card; if a queue
  that used to appear goes missing, map it in Dept Config (Operator
  State #14). **The internal
