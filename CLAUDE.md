...
- **The Insights report is the consolidation candidate for PR (and
  ...
  **No cache bump** -- `meta.agents` already carried the resolved selection,
  ...
  (deterministic per `hashAgents_` key; insights:v17 unchanged). The QCD
  ...
  **Chart consolidation (seq #1, insights:v17):** the 12-mo team-trend
  ...
  **Per-agent cards (seq #3, redesigned in the post-deploy pass):** each
  ...
  `insights:v17` already makes a fresh re-generate fast too). The team
...
S32 | QCD Report end-to-end | Subsystem: Department Dashboard + CDR Import
  ...
  Expected: own-dept Generate succeeds; cross-dept console call throws "Not authorized for this department.". Admin users can request any dept that exists in the dept list. Cache prefix `qcd:v10` keys are bounded (no agent-list dimension, so no MD5 hashing needed). My Department page renders a "Queue Call Data — [date]" tile row (showing the actual data date) below the agent table.
...
S36 | Dept Config modal: auto-discovery, validation, override round-trip | Subsystem: Department Dashboard
  Steps:
    ...
    - Re-open the QCD report for that dept -> the newly-mapped queue's rows now appear (after the qcd:v10 cache TTLs out). Re-open Overview -> a sub-queue mapping change is reflected immediately (the COMPANY_OVERVIEW_CACHE_KEY is busted on save).
