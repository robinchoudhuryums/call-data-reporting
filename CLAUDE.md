...
- **The Insights report is the consolidation candidate for PR (and
  ...
  **Per-agent cards (seq #3, redesigned in the post-deploy pass):** each
  ...
  `insights:v18` already makes a fresh re-generate fast too). The team
...
- **Anti-intimidation layer is client-only; keep it that way.** Four
  ...
  IR(single)/PR/CR/Insights (answer rate) + Inbound (abandon/answer);
  ...
  "abandons", since the QCD retirement) / Missed / Individual
  ...
  point), Missed auto-runs synchronously, Individual stops at the
...
- **Chart.js v4 + chartjs-plugin-datalabels needs explicit
  ...
  wired on IR / PR / CR / Insights; each persists it in its prefs blob
...
- **Multi-page architecture: Overview + My Department + Escalations.**
  ...
  Ranges, Alerts, Orphan Fix, Dept Config) overlay any page.
...
- **Top-tab router (Phase C).** The header nav was flattened from
  ...
  the report routes (`#/report/missed`, `#/report/individual`,
  `#/report/performance`, `#/report/compare`, `#/admin/alerts`,
  `#/admin/orphan-fix`; legacy `#/report/qcd` links repoint to the
  Insights modal since the QCD retirement) plus the three PAGE routes
