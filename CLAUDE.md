...
INV-30 | Each report has its own versioned cache key prefix; bump on any aggregation rule change so stale entries don't bleed in. Current: `summary:v4` (Data.gs), `latestDate:v1` (Data.gs — most-recent ISO date with data; drives the dashboard's default From/To), `individual:v6` (IndividualReport.gs), `individual_active:v1` (active-agents-in-range subset used by Individual + Performance + Compare Ranges pickers), `performance:v3` (PerformanceReport.gs), `compareRanges:v3` (CompareRangesReport.gs), `companyOverview:v8` (CompanyOverview.gs). Alerts.gs holds no cached compute. | Subsystem: Department Dashboard
