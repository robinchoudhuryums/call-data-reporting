...
- **The Insights report is the consolidation candidate for PR (and
  ...
  **Chart consolidation (seq #1, insights:v12):** the 12-mo team-trend
...
INV-55 | **Escalations (`Escalations.gs`) is the FIRST public PER-DEPT (non-admin) write path -- it extends INV-01.** Backed by the Neon `escalations` table (NOT a sheet; Phase 2 will let the external team-tools app INSERT `pending_review` rows into the same table, so Neon is the shared substrate). NO sheet fallback (like inbound_calls / Caller Lookup): unconfigured/unreachable Neon -> list renders an "unavailable" state, writes throw. Public callables: `getEscalationsInit` / `getEscalations` (read; manager pinned to own dept, admin may pass a dept or `'ALL'`; `getEscalations` is NOT cached -- a transient outage shouldn't pin an empty list), `createEscalation` (**admin-only**, `assertAdmin_` -- the "admin manually logs escalations" flow; validates known-dept + required reason), `resolveEscalation` + `updateEscalationComment` (**the per-dept write path** -- re-resolve caller + `assertDeptAccess_(user, <the row's OWN department, read from Neon, never trusted from the request>)`, so a manager touches only their own dept's rows). Four mitigations like the OrphanFix carve-out, admin gate swapped for the per-dept gate on the mutation paths: (1) authorization (above), (2) input validation (required fields, `ESC_MAX_TEXT`=4000 caps, `NULLIF(?, '')` for nullable cols so a blank optional field stores NULL without JDBC `setObject(null)`, and the business rule that **a resolution REQUIRES non-empty resolution text** -- you cannot mark resolved without explaining how, enforced server-side AND mirrored client-side by disabling the Resolved checkbox + Save until text is entered), (3) `LockService` on every write, (4) audit via the row's own created_by/created_at + resolved_by/resolved_at/updated_at columns (the immutable trail, queryable in Neon) + a Logger.log per action. All params are bound prepared-statement params (no SQL injection); the `escalations` table is created lazily (`CREATE TABLE IF NOT EXISTS` + a dept+status index) on first write, so no `setup()` change. Requires the dashboard `NEON_*` props + `script.external_request` scope. Client: an `Escalations` header tab (visible to managers + admins, route `#/escalations`) + modal; the `+ New escalation` create form is `data-admin-only`. Fields: Date & Time, Caller/Relation, Patient Name, Trx #, Area (optional), Reason; manager response = a Resolved checkbox (gated on a required resolution note) + optional Comments. New public write functions beyond escalations still need INV-01's mitigations; per-dept (non-admin) writes are sanctioned ONLY through this gate pattern. | Subsystem: Department Dashboard
...
S32 | QCD Report end-to-end | Subsystem: Department Dashboard + CDR Import
  Steps:
    ...
    - All-departments daily report (4b, admin-only): as an ADMIN on the Overview page, confirm a "Daily Call Queue Report" button (`#ov-qcd-alldept-btn`) is visible (hidden for managers). Click it; the `#qcd-alldept-modal` opens defaulting to "Yesterday". Generate; confirm a flat company-wide table with one section per mapped dept (own queues only -- a sub-queue lists under its own dept exactly once, never under the parent), a per-dept subtotal row, and a "Company total" grand-total footer; abandoned % >= 5% and violation counts are warn-tinted. Click Download CSV (scope line + per-dept rows + subtotals + grand total) and Print (plain-table print window). As a MANAGER, confirm the button is absent, and in the console `google.script.run.withFailureHandler(console.error).getQcdAllDepartments({from:'2026-05-01', to:'2026-05-19'})` throws "Alerts are admin-only." (the `assertAdmin_` gate).
