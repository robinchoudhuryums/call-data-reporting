...
25. `CONFIG_SOURCE` Script Property (dashboard) -- the C2 Dept Config
    ...
    **C3 data layer only:** Alert/Digest are HAND-EDITED today, so flipping
    them to `neon` makes them editable only via an admin UI that doesn't exist
    yet -- DO NOT set `CONFIG_SOURCE=neon` until those edit surfaces are built
    (the remaining C3 work). Dept Config (C2) is safe to flip now (it has the
    modal). Access Control is deliberately NOT in this flag -- it stays
    sheet-backed (auth availability), managed by the C1 editor.
