...
# that the sanitizeAbandonedCellForNeon_ function is identical across
# neonbackfill.js / NeonMirror.js (F-24). Non-zero exit on drift.
...
- **Public write paths are admin-only.** Three public surfaces write
  ...
  the canonical destination), `LockService` serialization (NB:
...
- **ATT semantics differ between the main dashboard and the per-agent
  ...
  ATT values (INV-05); the Individual, Compare Ranges, and Insights
  reports (and the retired Performance Report before them) use a
...
- **The Insights report ABSORBED the Performance Report (PR is
  RETIRED); CR remains the eventual consolidation candidate.**
...
- **Anti-intimidation layer is client-only; keep it that way.** Four
  ...
  **Benchmark tints** -- `benchValueCls_(label, formatted, symmetric)`
  ...
  values (IR/CR/Insights/Inbound) + inbound abandon-% cells. Default
...
- **Per-report client prefs in localStorage.** Each report persists its
  own form state under `cdr.ir.prefs.v1`,
  `cdr.cr.prefs.v1`, and `cdr.ins.prefs.v2` (the retired Performance
  Report's `cdr.pr.prefs.v1` is an orphan). Bump the trailing version when the prefs schema
...
- **Report SWR (stale-while-revalidate) rides the D1b keep-last-good
  ...
  repaint path clears its results-status line (IR/CR renderers clear
  ...
  Wired on: IR, CR (main Generate; the edit-popover path keeps its own
...
- **Chart.js v4 + chartjs-plugin-datalabels needs explicit
  ...
  wired on IR / CR / Insights; each persists it in its prefs blob
...
- **Neon read-back (F1) is flag-gated and defaults OFF.** The dashboard
  ...
  and **#4 the IR / CR / Insights builders** (the retired PR builder was the fourth). Each reads the windowed rows from
  ...
  **`computeActiveAgentsInRange_`** (the IR/CR/Insights agent-picker
...
- **Multi-page architecture: Overview + My Department + Escalations.**
  ...
  Settings, Individual / Performance / Compare Ranges / Insights,
...
- **Overview admin-only banners (Phase B).** Pipeline Health
  ...
  **Unmapped-queue nag (F onboarding):** a third admin-only banner
  ...
  `personalizeOverview_`; `companyOverview:v18`).
- **Top-tab router (Phase C).** The header nav was flattened from
  ...
  agent reports (IR / CR / Insights) the button also
...
- **Source column + roster-only totals (Phase D).** The agent table's
  ...
  **NOTE (Phase 14, commit 80e17da):** `getDepartmentSummary` now
  ...
  machinery survive for the IR/CR pickers (which DO still surface
...
19. `DQE_READ_SOURCE` Script Property (dashboard) -- the F1 Neon
    ...
    `getCompanyOverview`, `computeSummary_`, and the IR / CR /
...
22. Deferred Neon mirror (optional; `NeonMirror.js`, CDR Import). Defaults OFF
    ...
    (cdr-import, default 3000) is the window knob.
...
25. `CONFIG_SOURCE` Script Property (dashboard) -- the C2 Dept Config
    ...
    **C3 edit UIs SHIPPED:** the Alerts modal now has admin edit forms for both
    ...
    governs Dept + Alert + Digest config sources together.) All three
    compare gates read Neon DIRECTLY and return `clean:false` + an error
    when Neon is unreachable -- a Neon outage can no longer produce a
    false sheet-vs-sheet "PARITY CLEAN" (F-5); never flip the flag on a
    result that carries `error`. Access Control is
...
(INV-02/04/05/23/53, S35, E5), the IR/CR report builders + the Insights consolidation freeze (INV-25
...
INV-49 | `getIndividualReport` accepts optional `priorFrom`/`priorTo` for same-agent vs-self comparison. When supplied, every `summaryData[i]` carries `priorStats` (formatted) + `priorRaw` (numeric); `priorDateLabel` is set at the top level. Absence = legacy shape (`priorStats: null`). The cache key (`individual:v11`) adds a `priorKey` segment (`priorFrom..priorTo` or `none`) so the prior window is part of the cache identity. Client form (`ir-compare-mode` select) supports None / Same window one year prior / Immediately-preceding period / Custom prior range; resolved via `irResolvePriorRange_`. The same prior dates are re-applied automatically when the user re-runs from the edit-popover. | Subsystem: Department Dashboard
...
S4 | Missed Calls report (My Department inline section) renders for a known date | Subsystem: Department Dashboard
  Steps:
    ...
    - Also verify the two retained entry points: the Overview launcher chip "Why did we miss calls recently?" (sets the page dates to the latest DQE date, opens the dept page, and auto-scrolls to the section) and a `#/report/missed` deep link (same behavior).
...
S26 | Big-roster reports complete without cache-key error | Subsystem: Department Dashboard
  Steps:
    ...
    - Repeat for Compare Ranges and Insights with the same selection.
