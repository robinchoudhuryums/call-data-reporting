...
- **Neon write discipline (don't regress this — it caused a daily-import
  ...
  (L2)** -- the daily import + the per-date backfill/deferred path each pass
  ...
  `inbound_calls` (which has NO sheet primary; the DELETE spans the payload's
  distinct call_dates, in-txn; an extreme date-goes-to-ZERO-inbound re-import
  is the one corner it can't clear -- an empty payload carries no date to
  delete). Partial-set callers -- the bulk archive after
...
- **Force-path data-loss guard convention (M2 generalized).** A FORCE
  ...
  **The rule: a force-path writer that produces fewer rows than it deleted
  ...
  of these. Pinned by `csr-transfer.test.js` (the helper) + `pipeline-build.test.js` (M2).
...
- **Overview admin-only banners (Phase B).** Pipeline Health
  ...
  **Unmapped-queue nag (F onboarding):** a third admin-only banner
  ...
  `personalizeOverview_`; `companyOverview:v19`).
