...
INV-30 | Each report has its own versioned cache key prefix; bump on any aggregation rule change so stale entries don't bleed in. Current: `summary:v4` (Data.gs), `latestDate:v1` (Data.gs — most-recent ISO date with data; drives the dashboard's default From/To), `individual:v6` (IndividualReport.gs), `individual_active:v1` (active-agents-in-range subset used by Individual + Performance + Compare Ranges pickers), `performance:v3` (PerformanceReport.gs), `compareRanges:v3` (CompareRangesReport.gs), `companyOverview:v9` (CompanyOverview.gs), `qcd:v1` (QCDReport.gs). Alerts.gs holds no cached compute. | Subsystem: Department Dashboard
...
INV-50 | `QCD Historical Data` columns (1-indexed): `Month Year \| Week \| Date \| Call Queue \| Call Source \| Total Calls \| Total Answered \| Abandoned \| Longest Wait \| Avg Answer \| Abandoned % \| Violations`. Pinned in `Config.gs::QCD_HISTORICAL_COLS`. Writer: `apps-script/cdr-import/autoImport.js::processIntegratedHistory` QCD block. Reader: `apps-script/department-dashboard/QCDReport.gs` (dept-scoped report) + `CompanyOverview.gs::computeQcdSnapshots_` (per-dept latest-day snapshot on the Overview tile grid). `Call Queue` carries dept-name-like values ("CSR", "Sales", etc.) -- NOT raw queue names like "A_Q_CSR"; matches against dept-roster header byte-for-byte. `Call Source` is one of `Total Calls` (daily roll-up; the only source used for the per-day dept aggregate) / `CSR` / `Ad-campaign` / `New Call Menu` / `Non-CSR (internal)` -- source set pinned in `QCDReport.gs::QCD_SOURCES`. `Violations` is the count of (source, day) tuples where Abandoned % > 5%. | Subsystem: Department Dashboard + CDR Import
INV-51 | `QCD Report` (admin OR manager) is per-dept gated like Individual / Performance / Compare Ranges. `getQcdReport({ department, from, to })` returns `meta`, `dateLabel`, `totals` (Total Calls aggregate over range), `sourceBreakdown` (per-source row including a Total Calls roll-up), `trendData` (12-month monthly buckets matching the IR/PR trend window logic). Cache prefix `qcd:v1`. The Overview page's per-dept tile additionally shows a small QCD flag caption when the latest day's `Violations > 0` OR `Abandoned % >= 5%`; visible to everyone (no admin gate). | Subsystem: Department Dashboard
...
S32 | QCD Report end-to-end | Subsystem: Department Dashboard + CDR Import
  Steps:
    ...
    - Confirm per-source breakdown table shows the 5 sources from QCD_SOURCES with the Total Calls row bolded.
  ...
  Expected: own-dept Generate succeeds; cross-dept console call throws "Not authorized for this department.". Admin users can request any dept that exists in the dept list. Cache prefix `qcd:v1` keys are bounded (no agent-list dimension, so no MD5 hashing needed).
