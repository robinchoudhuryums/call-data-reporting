...
- **Admin emails are resolved at request time.** Membership checks
  ...
  Property's value wins there too). The constant
  `ADMIN_EMAILS_DISPLAY` in Config.gs is DEAD code -- nothing reads
  it (its own comment claims the mailto role; it doesn't have it) --
  and is a candidate for deletion. **Never read it (or any constant)
  for membership checks**; always go through `getAdminEmails_()`.
