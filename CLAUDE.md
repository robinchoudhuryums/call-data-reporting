...
30. `QCD_READ_SOURCE` Script Property (dashboard) -- the #3 Neon read-back
    ...
    stale mirror is visible before you flip. **Coverage (R-1): the flag gates
    ONLY the `computeQcdReport_` consumers** -- Insights Queue health + the
    all-dept Daily Call Queue Report (`qcdAll`). The Overview per-dept QCD
    chips (`computeQcdSnapshots_`), the My Department Queue Call Data snapshot
    (`computeDeptQcdSnapshot_`), and the freshness pill's QCD component
    (`getLatestDataDates`) are HARD-WIRED sheet scans and do NOT honor the
    flag -- if the QCD sheet is trimmed/aged after a Neon flip, those three
    silently go stale while Insights stays current. Do not retire the sheet's
    QCD history until they're routed through `readQcdGrid_` (fix queued,
    scan Batch 3). Needs the
